...
async function createActivities(arrayActivities) {
    try {
        ...
        for (let activity of arrayActivities) {
            let itinerary = await Itinerary.findOne({ name:activity.itinerary_id }) //Busco el itinerario que coincida con el nombre del objeto  
            let itinerary_id = await itinerary._id                                  //Necesito el id del itinerario
            activity.itinerary_id = itinerary_id                                    //Reasigno el valor de itinerary_id de cada actividad para cambiar el nombre que viene por defecto POR EL ID.
